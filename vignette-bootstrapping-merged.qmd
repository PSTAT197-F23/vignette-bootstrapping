...
A popular algortihm that uses this technique is XGBOOST. The techniques explained above are already implemented underthe hood, so utilizing boosting is fairly straight forward.

```{r}
pstat <- pstat %>% select(-dept)
```
Implementing XGboost in the common tidymodel library is very straight forward., and follows a standard procesior of recipe, workflow, training, and evaluation. 
```{r}
